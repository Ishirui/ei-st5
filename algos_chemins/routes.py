...
def deplacement_quadrillage(n, start, delivery_coords):
    g = quadrillage(n)
    d_c = [start[0]*n + start[1]] + [i[0]*n + i[1] for i in delivery_coords]
    it_sorties, pos_list = itineraire(g, d_c)
    print(it_sorties, pos_list)
    manoeuvres = []
    y = [i//n for i in pos_list]
    x = [i%n for i in pos_list]
    pos_counter = 1
    for i in it_sorties[1:]:
        print(manoeuvres)
        if i == -1:
